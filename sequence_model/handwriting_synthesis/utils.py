...
def plot_points(points, mu, sd, is_offset=True):
    # Convert points from a torch tensor to a numpy array
    points = points.cpu().numpy().squeeze()

    # Convert sd and mu to numpy arrays
    sd = sd.cpu().numpy()
    mu = mu.cpu().numpy()
    ...
    for point in points:
        ...
        if point[2] == 1:  # end_of_stroke
            # Plot the current stroke
            plt.plot(x_vals, y_vals)
            x_vals, y_vals = [], []  # Start a new stroke
        else:
            x_vals.append(x)
            y_vals.append(y)

    # Plot the final stroke if there is one
